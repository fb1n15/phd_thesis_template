...
\usepackage{amsthm}
\usepackage{caption}
\PassOptionsToPackage{hyphens}{url} 	% if you're using a class that already loads the package
\usepackage{hyperref}
...
%% ----------------------------------------------------------------
\begin{document}
\sloppy % prevent spilling into the margin
\frontmatter
...
\begin{algorithm}
    ...
    \For{$t$ in $T$}{
        ...
        (i.e., $ \underset{\lambda_j}{\argmax} \underset{j\in \theta_{flex}}{\sum}(\hat{v}_j(\lambda_j) - o_j(\lambda_j))$)  \Comment*[r]{Find the allocation for tasks in $ \theta_{flex} $ that maximise their social welfare} 
        ...
        \For{$ i $ in $\theta_{flex}$}{
            ...
            \For{$j$ = 1; $j \leq t_i$; $j$++ }{ \Comment*[r]{Update the valuation function of task $i$}
            $\hat{v}_i(j) \gets \hat{v}_i(j+ \underset{w \in W}{\sum}z_{w,t+1}^i)$ }  
            \If{$ t_i = 0 $}{
               $ \theta_{flex}  \gets \theta_{flex} \setminus \{i\} $ \Comment*[r]{Delete task $ i $ from flexible tasks if it gets its required usage time}
            }
            \If{$t = \hat{T}_i^d$}{
               $ \theta_{flex}  \gets \theta_{flex} \setminus \{i\} $ \Comment*[r]{Delete task $ i $ from flexible tasks if it reaches its deadline}
            }
        }
...
This mechanism is an extension of the greedy algorithm from~\cite{wang2012cloud}, which greedily make an allocation decision to maximise the utility of a task when it arrives and commits to the decision henceforth. Furthermore, it computes the payment as this task's corresponding operational costs ($ p_i= o_i = \sum_{r \in R,w \in W, t\in T} (a_{i,r} z_{w,t}^i o_{w,r}) + \sum_{l \in L,w \in W,(j,k) \in \mathbb{E}, t\in T} (2  o_{j,k}  f_{l,w, j,k,t}^i) $). This mechanism is DSIC and IR, and the details of this mechanism are given in Algorithm \ref{alg: og}:
...
\begin{algorithm}
    ...
    \For{$t$ in $T$}{
        \While{ new tasks arrive within $ t $}{
            ...
            (i.e., $\underset{\lambda_i}{\argmax} \underset{t\in T}{\sum} (\hat{v_i}  - o_i(\lambda_i))$, given $ \Lambda $ \& $ \tilde{\theta}_i $) \Comment*[r]{Find the allocation for task $ i $ that maximise its utility}
...
\begin{algorithm}
    ...
    \For{$t$ in $T$}{
        ...
        \For{$ i $ in $\theta_{flex}$}{
            ...
            \For{$j$ = 1; $j \leq \tilde{t}_i$; $j$++ }{ \Comment*[r]{Update the valuation function of task $i$}
            $\hat{v}_i(j) \gets \hat{v}_i(j+ \underset{w \in W}{\sum}z_{w,t+1}^i)$ }  
            \If{$ \tilde{t}_i = 0 $}{
               $ \theta_{flex}  \gets \theta_{flex} \setminus \{i\} $ \Comment*[r]{Delete task $ i $ from the set of flexible tasks if it gets its required usage time}
            }
            \If{$t = \hat{T}_i^d$}{
               $ \theta_{flex}  \gets \theta_{flex} \setminus \{i\} $ \Comment*[r]{Delete task $ i $ from the set of flexible tasks if it reaches its deadline}
            }
        }
